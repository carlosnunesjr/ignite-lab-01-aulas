...
class Merger {
    ...
    validateOverride(sources, { coordinate }) {
        ...
        subgraphsWithOverride.forEach((subgraphName) => {
            ...
            if (!this.names.includes(sourceSubgraphName)) {
                ...
                this.errors.push(federation_internals_1.ERRORS.OVERRIDE_FROM_SELF_ERROR.err(`Source and destination subgraphs "${sourceSubgraphName}" are the same for overridden field "${coordinate}"`, { nodes: overrideDirective === null || overrideDirective === void 0 ? void 0 : overrideDirective.sourceAST }));
            }
    ...
    validateFieldSharing(sources, dest, mergeContext) {
        ...
        if (nonShareableSources.length > 0 && (shareableSources.length > 0 || nonShareableSources.length > 1)) {
            ...
            const nonShareables = shareableSources.length > 0
                ...
                : 'all of them';
            this.errors.push(federation_internals_1.ERRORS.INVALID_FIELD_SHARING.err(`Non-shareable field "${dest.coordinate}" is resolved from multiple subgraphs: it is resolved from ${(0, federation_internals_1.printSubgraphNames)(resolvingSubgraphs)} and defined as non-shareable in ${nonShareables}`, { nodes: (0, federation_internals_1.sourceASTs)(...allResolving) }));
        }
    }
    ...
    recordAppliedDirectivesToMerge(sources, dest) {
        const inaccessibleInSupergraph = this.mergedFederationDirectiveInSupergraph.get(inaccessibleSpec.inaccessibleDirectiveSpec.name);
        const inaccessibleName = inaccessibleInSupergraph === null || inaccessibleInSupergraph === void 0 ? void 0 : inaccessibleInSupergraph.name;
        ...
        if ((_a = dest.schema().directive(name)) === null || _a === void 0 ? void 0 : _a.repeatable) {
            ...
            if (differentApplications.length === 1) {
                ...
                this.mismatchReporter.reportMismatchHint({
                    ...
                    message: `Non-repeatable directive @${name} is applied to "${(_b = dest['coordinate']) !== null && _b !== void 0 ? _b : dest}" in multiple subgraphs but with incompatible arguments. `,
                    supergraphElement: dest,
                    subgraphElements: sources,
                    elementToString: (elt) => {
                        var _a;
